...
class Disc(db.Model):
    ...
    ocors = db.relationship('Ocor', backref='disc', lazy='dynamic')
...
def ocorrencias():
    form = NameForm()
    # Ordena as ocorrências por data em ordem decrescente
    ocor_all = Ocor.query.order_by(desc(Ocor.date)).all()
    discs = Disc.query.all()  # Para listar todas as disciplinas já salvas
    
    if form.validate_on_submit():
        # Verificar se a disciplina já existe
        ocor_disc = Disc.query.filter_by(name=form.disc.data).first()

        # Se a disciplina existir, associar à ocorrência
        if ocor_disc:
    ...
    return render_template('ocorrencias.html', form=form, ocor_all=ocor_all, discs=discs)   
